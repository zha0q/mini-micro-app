...
var DocumentCloner = /** @class */ (function () {
  ...
  DocumentCloner.prototype.appendChildNode = function (
    ...
    if (child.className === 'rame-loading') return;
    if (
      !isElementNode(child) ||
      ...
      if (
        ...
        if (child.tagName == 'IFRAME') {
          const node = iframeElementWeakMap.get(child);
          clone.appendChild(node);
        } else clone.appendChild(this.cloneNode(child, copyStyles));
      }
    }
  };
  DocumentCloner.prototype.cloneChildNodes = function (
    ...
    for (
      ...
      // if(node.shadowRoot) console.log(child, child.nextSibling);
      if (
        isElementNode(child) &&
        isSlotElement(child) &&
...
var iframeElementWeakMap;

var html2canvas = function (element, options, iframeElementProps) {
  iframeElementWeakMap = iframeElementProps;
  if (options === void 0) {
    options = {};
  }
